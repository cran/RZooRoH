...
lik_mixkl <- function(zpar,zooin,pem, zrs) {
  K <- length(zpar)+1
  fs=array(0,K)
#  D=1
...
fb_lay <- function(zooin,id,pem,zrates,zmix,localhbd = FALSE) {
  K <- length(zmix)
...
viterbi_lay <- function(zooin,id,pem,zrates,zmix) {
  K <- length(zmix)
  decout <- .Fortran("zoolayerViterbi",as.integer(K), as.integer(zooin@nchr),as.integer(zooin@nsnps),
...
### Run EM algorithm
EM_lay <- function(zooin,id, pem, zrates, zmix, estimr = 1, oner = 0, nrounds = 1000, minr = 1, maxr = 1024, convem = 1e-10, localhbd = FALSE) {
  K <- length(zmix)
  ems <- .Fortran("zoolayerEM",as.integer(K), as.integer(zooin@nchr),as.integer(zooin@nsnps),
                  as.double(pem), as.integer(zooin@chrbound),as.double(zrates),as.double(zmix),
                  as.integer(zooin@bp),as.double(0),matrix(as.double(0),K,zooin@nsnps),
