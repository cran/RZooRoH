...
fb <- function(id,zrates,zmix, localhbd = FALSE) {
  K <- length(zmix)
  onefb <- .Fortran("zooFB",as.integer(K), as.integer(.GlobalEnv$zooin@nchr),as.integer(.GlobalEnv$zooin@nsnps),
                    as.double(.GlobalEnv$pem),
                    as.integer(.GlobalEnv$zooin@chrbound),as.double(zrates),as.double(zmix),
                    as.integer(.GlobalEnv$zooin@bp),as.double(0),matrix(as.double(0),K,.GlobalEnv$zooin@nsnps))
...
viterbi <- function(id,zrates,zmix) {
  K <- length(zmix)
  decout <- .Fortran("zooViterbi",as.integer(K), as.integer(.GlobalEnv$zooin@nchr),as.integer(.GlobalEnv$zooin@nsnps),
                     as.double(.GlobalEnv$pem), as.integer(.GlobalEnv$zooin@chrbound),as.double(zrates),as.double(zmix),
                  as.integer(.GlobalEnv$zooin@bp),array(as.integer(0),.GlobalEnv$zooin@nsnps))
  ...
  for (chr in 1:.GlobalEnv$zooin@nchr){
    chrstates <- states[.GlobalEnv$zooin@chrbound[chr,1]:.GlobalEnv$zooin@chrbound[chr,2]]
    posbp <- .GlobalEnv$zooin@bp[.GlobalEnv$zooin@chrbound[chr,1]:.GlobalEnv$zooin@chrbound[chr,2]]
    ns <- length(chrstates)
...
### Run EM algorithm
EM <- function(id, zrates, zmix, estimr = 1, oner = 0, nrounds = 1000, minr = 1, maxr = 1024, convem = 1e-10, localhbd = FALSE) {
  K <- length(zmix)
  ems <- .Fortran("zooEM",as.integer(K), as.integer(.GlobalEnv$zooin@nchr),as.integer(.GlobalEnv$zooin@nsnps),
                  as.double(.GlobalEnv$pem), as.integer(.GlobalEnv$zooin@chrbound),as.double(zrates),as.double(zmix),
                  as.integer(.GlobalEnv$zooin@bp),as.double(0),matrix(as.double(0),K,.GlobalEnv$zooin@nsnps),
